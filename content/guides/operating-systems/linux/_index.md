---
title: Linux
---

Guides covering Linux distributions, administration, shell, and system configuration.

{{< cards >}}
  {{< card link="ssh-keys-linux" title="SSH Keys – The Right Way" icon="shield-check" >}}
  {{< card link="ufw-firewall-linux" title="UFW Firewall on Ubuntu and Debian" icon="shield-check" >}}
  {{< card link="essential-linux-commands" title="Essential Linux Commands" icon="terminal" >}}
  {{< card link="aliases-linux" title="Bash Aliases on Linux" icon="cog" >}}
  {{< card link="mount-disk-linux-fstab" title="Mount a Disk in Linux (fstab)" icon="server" >}}
  {{< card link="mount-network-share-linux" title="Mount a Network Share in Linux (fstab)" icon="globe-alt" >}}
  {{< card link="samba-file-sharing-linux" title="Set Up Samba File Sharing on Linux" icon="server" >}}
{{< /cards >}}
